...
const EditContact = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const res = await fetch(`${import.meta.env.REACT_APP_BACKEND_URL}/api/contact`, {
      method: "PUT",
      headers: {
  ...
  const editEffect=async () => {
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.REACT_APP_BACKEND_URL}/api/contact/${id}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
